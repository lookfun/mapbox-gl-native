...
TEST(Annotations, Points) {
    ...
    layer.add(std::make_shared<const Annotation>(AnnotationType::Point, "default_marker", LatLng{ 13, 52 }));

    {
        const auto result = layer.getTile(TileID{ 0, 0, 0, 1 });
        EXPECT_EQ(1u, result.size());
        EXPECT_DOUBLE_EQ(13, result[0]->point.latitude);
        EXPECT_DOUBLE_EQ(52, result[0]->point.longitude);
    }
    {
        const auto result = layer.getTile(TileID{ 1, 0, 0, 1 });
        EXPECT_EQ(0u, result.size());
    }
    {
        const auto result = layer.getTile(TileID{ 1, 1, 0, 1 });
        EXPECT_EQ(1u, result.size());
        EXPECT_DOUBLE_EQ(13, result[0]->point.latitude);
        EXPECT_DOUBLE_EQ(52, result[0]->point.longitude);
    }
    {
        const auto result = layer.getTile(TileID{ 1, 0, 1, 1 });
        EXPECT_EQ(0u, result.size());
    }
    {
        const auto result = layer.getTile(TileID{ 1, 1, 1, 1 });
        EXPECT_EQ(0u, result.size());
    }
}
...
TEST(Annotations, Points2) {
    ...
    for (size_t i = 0; i < total; i++) {
        ...
        annotations.emplace(std::make_shared<const Annotation>(AnnotationType::Point, "default_marker", LatLng{ lat, lon }));
    }
    ...
    {
        const auto result = layer.getTile(TileID{ 0, 0, 0, 0 });
        EXPECT_EQ(total, result.size());
    }

    const auto result_100 = layer.getTile(TileID{ 1, 0, 0, 1 });
    const auto result_101 = layer.getTile(TileID{ 1, 0, 1, 1 });
    const auto result_110 = layer.getTile(TileID{ 1, 1, 0, 1 });
    const auto result_111 = layer.getTile(TileID{ 1, 1, 1, 1 });
...
TEST(Annotations, Points3) {
    ...
    layer.add(std::make_shared<const Annotation>(AnnotationType::Point, "default_marker", LatLng{ 52.520803, 13.40945 }));
    ...
    for (int z = 18, x = 140836, y = 85970; z > 0 ; --z, x /= 2, y /= 2) {
        const auto result = layer.getTile(TileID{ int8_t(z), x, y, int8_t(z) });
        EXPECT_EQ(1u, result.size());
    }
}
